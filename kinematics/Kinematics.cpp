...
TableArmKinematics::TableArmKinematics()
{

    Initialize("tabla_170_grados.csv", vector<int>{171,360});
}



TableArmKinematics::TableArmKinematics(string path)
{

    Initialize(path, vector<int>{171,360});
}

long TableArmKinematics::GetIK(long theta, long phi, vector<double> & lengths)
{
    //  cout << lookupIndex.size();
    long index = lookupIndex[theta][phi];
    if (lengths.size()!=lookupTable[index].size())
...
long TableArmKinematics::Initialize(string csvfileName, vector<int> tableDimensions)
{
...
ArmInvKinematics::ArmInvKinematics()
{
...
ArmInvKinematics::ArmInvKinematics(double new_a, double new_b, double new_L0)
{
    a=new_a;//m distance between A and base
    b=new_b;//m distance between B and mobile platform
    L0=new_L0;// Arm Lenght
}

long ArmInvKinematics::GetIK(double incl, double orien, vector<double> &lengths)
{
...
//    if (incl == 0) return -1;
    ...
    lengths[0]=L0 - theta * a * cos(phi1);
    lengths[1]=L0 - theta * a * cos(phi2);
    lengths[2]=L0 - theta * a * cos(phi3);
    }
    else
    {
    lengths[0]=L0;
