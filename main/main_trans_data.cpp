...
int main ()
{
    ...
    ofstream data("/home/humasoft/code/Soft-Arm/graphs/Path_trans_data5.csv",std::ofstream::out); // /home/humasoft/code/graficas
    ...
    double seg_ref=freq*2.5;
    double cnt_ref=0;
    double time_test=200;
    for (int bub=0;bub<10;bub+=1){
        for (double t=0;t<time_test;t+=dts)
        {
            ...
            data<< t << " , " <<  cs[0] << " , " <<cs[1]<< " , " << roll << " , " << pitch << " , " << yaw<<" , " <<  m1.GetPosition() <<" , " <<m2.GetPosition() <<" , " <<m3.GetPosition() << endl; //CR
            //cout << endl;
            Ts.WaitSamplingTime();
        }
        ...
        for (double t=0;t<3;t+=dts)
        {
            ...
            m3.SetPosition(0);
            misensor.GetPitchRollYaw(pitch,roll,yaw);
            data<< t+time_test << " , "  <<  cs[0] << " , " <<cs[1]<< " , " << roll << " , " << pitch << " , " << yaw<<" , " <<  m1.GetPosition() <<" , " <<m2.GetPosition() <<" , " <<m3.GetPosition() << endl; //CR
            //cout << endl;
            Ts.WaitSamplingTime();
        }
    }
